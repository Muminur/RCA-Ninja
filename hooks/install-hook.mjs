...
// post-merge is what catches a GitHub squash merge: the squash commit is
// created on the server, so it never passes through post-commit, and it only
// reaches a local hook when the merge arrives via git pull / git merge.
