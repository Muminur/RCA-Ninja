...
describe('hooks', () => {
  ...
  (process.platform === 'win32' ? it.skip : it)(
    'install-hook.sh does not write through a hooks directory swapped during validation',
    () => {
      const { home, gitconfig, initialConfig, env } = makeIsolatedGitEnv(
        'claude-rca-hook-dir-swap-',
      );
  ...
  (process.platform === 'win32' ? it.skip : it)(
    'install-hook.sh rejects when the hooks path links to the moved directory',
    () => {
      const { home, gitconfig, initialConfig, env } = makeIsolatedGitEnv(
  ...
  (process.platform === 'win32' ? it.skip : it)(
    ...
    () => {
      ...
      writeFileSync(join(mvDir, 'mv'), `#!/bin/sh\n"${failMv}" "$@"\n`, 'utf8');
      try {
