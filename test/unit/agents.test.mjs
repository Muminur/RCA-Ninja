...
describe('subagents', () => {
  it('agents directory has at least 2 agent files', () => {
    const files = readdirSync(AGENTS_DIR).filter((f) => f.endsWith('.md'));
    assert.ok(files.length >= 2, `Expected ≥2 agent files, got ${files.length}`);
  });

  it('all agent files have valid YAML frontmatter', () => {
  ...
  it('all agent files have a name field', () => {
    for (const file of readdirSync(AGENTS_DIR).filter((f) => f.endsWith('.md'))) {
  ...
  it('all agent files have a description field', () => {
    for (const file of readdirSync(AGENTS_DIR).filter((f) => f.endsWith('.md'))) {
      const { fm } = parseAgentFile(join(AGENTS_DIR, file));
  ...
  it('model field when present must be sonnet, haiku, or opus', () => {
    for (const file of readdirSync(AGENTS_DIR).filter((f) => f.endsWith('.md'))) {
      const { fm } = parseAgentFile(join(AGENTS_DIR, file));
  ...
  it('code-reviewer.md exists with correct name and tools fields', () => {
